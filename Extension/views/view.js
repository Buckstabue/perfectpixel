        ...
        .extend({
            ...
            initialize : function(options) {
                ...
                ExtensionService.sendMessage({
                    type : PP_RequestType.getTabId
                }, function(res) {
                    view.model = new Panel({
                        id : res.tabId
                    });
                    view.model.fetch();
            ...
            updatePanel : function(obj) {
                ...
                for ( var index in position) {
                    if (position[index] == 0)
                        this.$el.addClass('attached-' + index)
                }
            ...
            upload : function(file) {
                ...
                overlay.uploadFile(file, $.proxy(function() {
                    ...
                    if (ExtOptions.NewLayerMoveToScrollPosition)
                        overlay.set('y', $(window).scrollTop());
                    overlay.save();

                    if (!PerfectPixel.getCurrentOverlay() || ExtOptions.NewLayerMakeActive) {
                        PerfectPixel.setCurrentOverlay(overlay);
                    }
                    if (ExtOptions.NewLayerShow)
                        PerfectPixel.showOverlay();
                    if (ExtOptions.NewLayerUnlock)
                        PerfectPixel.unlockOverlay();
                }, this));
            },
            ...
            originButtonClick : function(e) {
                ...
                if (overlay) {
                    ...
                    if (e.shiftKey)
                        offset *= this.fastMoveDistance;
                    if (axis == "x") {
                        PerfectPixel.moveCurrentOverlay({
                            x : overlay.get('x') - offset
                        });
                    } else if (axis == "y") {
                        PerfectPixel.moveCurrentOverlay({
                            y : overlay.get('y') - offset
                        });
                    }
                }
            },
            ...
            changeOrigin : function(e) {
                ...
                if (overlay) {
                    ...
                    switch (axis) {
                    case 'x':
                        var currentValue = PerfectPixel.moveCurrentOverlay({
                            x : value
                        });
            ...
            changeOpacity : function(e) {
                if (this.$(e.currentTarget).is(":disabled")) { // chrome bug if
                    // version <
                    // 15.0; opacity
                    // input isn't
                    // actually
                ...
                if (overlay) {
                    ...
                    var value = input.val();
                    var returnValue = PerfectPixel.changeCurrentOverlayOpacity({
                        opacity : Number(value).toFixed(1)
                    });
            ...
            onOpacityChanged : function(e) {
                trackEvent("opacity", e.type, e.currentTarget.value * 100); // GA
                // tracks
                // only
                // integers
                // not
            ...
            changeScale : function(e) {
                ...
                var overlay = PerfectPixel.getCurrentOverlay();
                if (overlay) {
                    var returnValue = PerfectPixel.scaleCurrentOverlay({
            ...
            keyDown : function(e) {
                if ($(e.target).is('.title[contenteditable]'))
                    return;
                var overlay = PerfectPixel.getCurrentOverlay();
            ...
            update : function() {
                ...
                this.$('.chromeperfectpixel-showHideBtn').button({
                    disabled : isNoOverlays
                });
                this.$('.chromeperfectpixel-lockBtn').button({
                    disabled : isNoOverlays
                });
                this.$('.chromeperfectpixel-lockBtn span').text(
                        PerfectPixel.get('overlayLocked') ? ExtensionService.getLocalizedMessage('unlock')
                                : ExtensionService.getLocalizedMessage('lock'));
                this.$('.chromeperfectpixel-min-lockBtn').text(PerfectPixel.get('overlayLocked') ? 'l' : 'u');
                this.$('.chromeperfectpixel-invertcolorsBtn').button({
                    disabled : isNoOverlays
                });
                this.$('.chromeperfectpixel-invertcolorsBtn span').text(
                        PerfectPixel.get('overlayInverted') ? ExtensionService.getLocalizedMessage('uninvert_colors')
                                : ExtensionService.getLocalizedMessage('invert_colors'));
                this.$('#chromeperfectpixel-origin-controls button').button({
                    disabled : isNoOverlays
                });
            ...
            updateNotification : function() {
                var myNotify = PerfectPixel.notificationModel.getCurrentNotification(), box = $('#chromeperfectpixel-notification-box'), textDiv = $('#chromeperfectpixel-notification-text'), button = $('#chromeperfectpixel-closeNotification');
                if (myNotify) {
            ...
            togglePanelShown : function() {
                ...
                ExtensionService.sendMessage({
                    type : PP_RequestType.PanelStateChange,
                    state : new_state
                });
            },
            ...
            render : function() {
                ...
                var panelHtml = '<div id="chromeperfectpixel-dropzone-decorator"></div>'
                        ...
                        + '"></div>'
                        + '<h1>'
                        + ExtensionService.getLocalizedMessage("extension_name_short")
                        + '</h1>'
                        + '</div>'
                        ...
                        + '<span>'
                        + ExtensionService.getLocalizedMessage("opacity")
                        + ':</span>'
                        ...
                        + '<span>'
                        + ExtensionService.getLocalizedMessage("origin")
                        + ':</span>'
                ...
                // Workaround to catch single value of opacity during opacity
                // HTML element change
                (function(el, timeout) {
                    var prevVal = el.val();
                ...
                this.$el
                        .draggable({
                            ...
                            stop : function(event, ui) {
                                var $window = $(window), screenWidth = $window.width(), screenHeight = $(
                                        '#' + view.screenBordersElementId).height(), $panel = $(event.target), position = {
                                    left : ui.position.left,
                                    top : ui.position.top,
                                    right : screenWidth - (ui.position.left + $panel.width()),
                                    bottom : screenHeight - (ui.position.top + $panel.height())
                                }, outOfBoundaries = false, new_params = {};
                                ...
                                if (panelModel.get('collapsed') && panelModel.get('auto_collapsed')
                                        && position.top != 0 && position.right != 0 && position.bottom != 0
                                        && position.left != 0) {
            ...
            _initDropzone : function() {
                ...
                dropzone.on("drop", function(e) {
                    ...
                    /*
                     * try { var length =
                     * e.originalEvent.dataTransfer.items.length; for (var i =
                     * 0; i < length; i++) { var entry =
                     * e.originalEvent.dataTransfer.items[i].webkitGetAsEntry();
                     * if (entry && (entry.isFile || entry.isDirectory)) {
                     * trackEvent("dropzone", e.type, entry.isFile ? "file" :
                     * "directory"); } } } catch(e) {}
                     */
                });
...
var OverlayItemView = Backbone.View.extend({
    ...
    titleKeyDown : function(e) {
        ...
        if (e.keyCode == 13) {
            ...
            if (s.extentOffset == s.baseOffset)
                return false; // when (s.extentOffset != s.baseOffset) text is
            // selected
        }
    },
    ...
    render : function() {
        if (this.$el.find('.chromeperfectpixel-delete').size() == 0) {
            ...
            if (title)
                this.$el.append($(this.title_template).text(title));
        }

        this.model.image.getThumbnailUrlAsync($.proxy(function(thumbUrl) {
            thumbUrl && this.$el.css({
    ...
    remove : function() {
        var deleteLayerConfirmationMessage = ExtensionService
                .getLocalizedMessage('are_you_sure_you_want_to_delete_layer');
...
var OverlayView = Backbone.View
        .extend({
            ...
            updateOverlay : function() {
                ...
                this.$el.css('width', width + 'px').css('left', this.model.get('x') + 'px').css('top',
                        this.model.get('y') + 'px').css('opacity', this.model.get('opacity'));
            ...
            mousewheel : function(e) {
                if (ExtOptions.enableMousewheelOpacity) {
                    if (e.originalEvent.wheelDelta < 0) {
                        this.model.save({
                            opacity : Number(this.model.get('opacity')) - 0.05
                        });
                    } else {
                        this.model.save({
                            opacity : Number(this.model.get('opacity')) + 0.05
            ...
            startDrag : function(e, ui) {
                // If focus is on PP panel input's remove it to allow arrow
                // hotkeys work on overlay
                var focusedElem = $(getFocusedElement());
                if (focusedElem
                        && (focusedElem.is('input#chromeperfectpixel-opacity')
                                || focusedElem.is('input#chromeperfectpixel-coordX') || focusedElem
                                .is('input#chromeperfectpixel-coordY'))) {
                    focusedElem.blur();
                }

                // For Smart movement
                ui.helper.data('PPSmart.originalPosition', ui.position || {
                    top : 0,
                    left : 0
                });
                    ...
                    if (e.shiftKey === true) {
                        ...
                        if (stickBorder == null) {
                            ...
                            if (Math.abs(deltaX) >= Math.abs(deltaY)) {
                                stickBorder = {
                                    x : this.smartMovementStickBorder,
                                    y : 0
                                };
                            } else {
                                stickBorder = {
                                    x : 0,
                                    y : this.smartMovementStickBorder
                                };
                            }
                            ui.helper.data('PPSmart.stickBorder', stickBorder);
                        }

                        // console.log("X: " + deltaX + "; stickBorderX: " +
                        // stickBorder.x + " Y: " + deltaY + "; stickBorderY: "
                        // + stickBorder.y);
